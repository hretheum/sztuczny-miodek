...
SCHEMAT METRYK (nazwy, typy, etykiety) — patrz metrics-exporter.schema.md (korzeń repo).
"""
...
from miodek import metrics
from miodek import config as _config
from miodek import decision_log
from miodek import runner

# Linter jako moduł pakietu (KAN-228): metrics_exporter jest częścią pakietu, więc subprocess
# -m miodek.ai_linter dziedziczy środowisko (po instalacji pakiet na ścieżce, z repo PYTHONPATH=src).
...
def _corpus_paths(corpus):
    ...
    *.md/*.txt) i globy, więc przekazujemy ścieżkę jak jest. Pusta/niepodana => katalog roboczy
    (eksporter jako pakiet nie zna „katalogu repo"; deploy podaje --corpus jawnie)."""
    if not corpus:
        return [os.getcwd()]
    return [corpus]
...
def serve(host, port, corpus, log_path, profile, dict_path, lang, ttl):
    ...
    sys.stderr.write(
        ...
        f"(korpus={corpus or os.getcwd()}, log={log_path}, ttl={ttl}s)\n"
    )
    try:
        httpd.serve_forever()
